...
datasets = ['Train', 'Playground', 'M60', 'Truck']
datasets_renames = ['tat_all_intermediate_Train_0.5_n6', 'tat_all_intermediate_Playground_0.5_n6', 'tat_all_intermediate_M60_0.5_n6', 'tat_all_training_Truck_0.5_n6']
paths = [
    '../exp/experiments/backbone_perceptual_gan_ref_6_patch_[80, 160]_complement/',
         ]
gt_path = '../GARS/Tanks_and_Temples_HLBIC_FR_MVRs_NVRs/LR/x4/'
gt_subpaths = ['intermediate/Train', 'intermediate/Playground', 'intermediate/M60', 'training/Truck']
save_paths = [
    '../exp/experiments/backbone_perceptual_gan_ref_6_patch_[80, 160]_complement/',
         ]
iter =  [ str(args.iter), ]
sr_names = [ 'tanks_&_temples', ]
save_every = args.save_every

if not os.path.exists(args.result_path):
    os.makedirs(args.result_path)
file_txt=open(os.path.join(args.result_path, 'tanks_&_temples.txt'),mode='w')
...
for path_count, path in enumerate(paths):
    ...
    for dataset_count, dataset in enumerate(datasets):
        ...
        gt_l = []
        subdir = os.path.join(gt_path, gt_subpaths[dataset_count], 'dense', 'ibr3d_pw_0.50')
        subsubdirs = os.listdir(subdir)
        subsubdirs.sort()
