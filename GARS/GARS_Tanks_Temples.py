...
parser.add_argument("--path", type=str, default="./Tanks_and_Temples_HLBIC_FR/LR/x4/intermediate/")
parser.add_argument("--save_path", type=str, default="./Tanks_and_Temples_HLBIC_FR_MVRs_NVRs/LR/x4/intermediate/")
...
for subpath in subpaths:
    ...
    scene_use_cuda = False
    if use_cuda == True and '/Courthouse' not in root and '/Palace' not in root and '/Church' not in root:
        scene_use_cuda = True
    ...
    for i, dm in enumerate(tqdm(dm_paths, ncols=50)):
        ...
        for candidate in range(6):
            ...
            index_clone = index.clone()
            index = torch.unsqueeze(index, dim=2).repeat(1, 1, 3, 1, 1)
            near_view_reference = torch.unsqueeze(align_pixel_clone, dim=0)
            near_view_reference = torch.gather(near_view_reference, 1, index)
            near_view_reference = torch.squeeze(near_view_reference).permute(1, 2, 0)
            near_view_reference = PIL.Image.fromarray(near_view_reference.cpu().numpy().astype(np.uint8))
            near_view_reference.save(os.path.join(save_path_ref, 'refer_near_' + str(candidate).zfill(2) + '.png'))
            index = torch.unsqueeze(index_clone, dim=2)
            align_depth.scatter_(dim=1, index=index, src=scatter_src)
